...
function createCurveControlPoints(t, e) {
  ...
  var a = null,
    ...
    o = null,
    r = null;
  if (e < 1 ? (a = t[0].x + .2 * (t[1].x - t[0].x), n = t[0].y + .2 * (t[1].y - t[0].y)) : (a = t[e].x + .2 * (t[e + 1].x - t[e - 1].x), n = t[e].y + .2 * (t[e + 1].y - t[e - 1].y)), e > t.length - 3) {
    var s = t.length - 1;
...
function getYAxisTextList(t, e, i) {
  ...
  for (var s = getDataRange(n, o), l = s.minRange, h = s.maxRange, c = [], d = (h - l) / i.yAxisSplit, x = 0; x <= i.yAxisSplit; x++) c.push(l + d * x);
  return c.reverse()
}
...
function drawPieText(t, e, i, a, n, o) {
  ...
  t.map(function(t) {
    ...
    l && util.isSameXCoordinateArea(l.start, {
      x: c
    }) && (f = c > 0 ? Math.min(d, l.start.y) : e < 0 ? Math.max(d, l.start.y) : d > 0 ? Math.max(d, l.start.y) : Math.min(d, l.start.y)), c < 0 && (c -= x);
    var u = {
      lineStart: {
        x: o,
        y: h
...
function drawYAxisTitle(t, e, i, a) {
  ...
  a.save(), a.beginPath(), a.setFontSize(i.fontSize), a.setFillStyle(e.yAxis.titleFontColor || "#333333"), a.translate(0, e.height), a.rotate(-90 * Math.PI / 180), a.fillText(t, n, i.padding + .5 * i.fontSize), a.stroke(), a.closePath(), a.restore()
}
  ...
  var e = function() {
    ...
    a = function(n) {
      ...
      if (null === i && (i = n), n - i < t.duration) {
        ...
        o = (0, Timing[t.timing])(o), t.onProcess && t.onProcess(o), e(a, 17)
