...
// 本地服务器
export const BASE_URL = 'http://192.168.0.144:8099/api/'
export const DOMAIN_NAME = 'http://localhost:8080'
export const BASE_HTTPS_URL = 'https://www.easy-mock.com/mock/5aacc9a1d3f6bd35dfb4be65/api/v1/'
    /**
     * get 方法
...
export function request(method, relativeUrl, param, header) {
    ...
    return new Promise((resolve, reject) => {
        wx.request({
            ...
            success(res) {
                response = res.data
                if (res.statusCode < 400 && res.data.ApiStatus != 0) {
                    resolve(res)
                } else {
                    reject(res)
                }
